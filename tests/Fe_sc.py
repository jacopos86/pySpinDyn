...
#
crystal = ase.build.bulk('Fe', 'fcc', a=3.571)
V = crystal.get_volume()
# pyscf cell
unit_cell = pbcgto.Cell()
unit_cell.atom = pyscf_ase.ase_atoms_to_pyscf(crystal)
unit_cell.a = crystal.cell
#unit_cell.basis = gto.load(basis_set_exchange.api.get_basis('STO-3G', elements='Fe', fmt='nwchem'), 'F')
#unit_cell.exp_to_discard=0.1
unit_cell.basis = 'sto-3g'
unit_cell.pseudo = 'gth-blyp'
unit_cell.spin = 2
unit_cell.verbose = 5
unit_cell.build(None,None)
#unit_cell.ke_cutoff = 10.0
...
#
mf = pbcdft.KRKS(unit_cell).density_fit()
